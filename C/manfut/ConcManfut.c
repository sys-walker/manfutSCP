...
void* CalcularEquipOptim_thread(Intervals *pIntervals);
...
int main(int argc, char *argv[])
{
    ...
    if (argc<3)
        error("Error in arguments: ManFut <presupost> <fitxer_jugadors> <Numthreads>");
    if (argc>3){
        num_threads =  atoi(argv[3]);
    } else{
        write(1,color_red,strlen(color_red));
        write(1,"No shan introduit cap thread, per defecte num_threads=2\n\033[00m",strlen("No shan introduit cap thread, per defecte num_threads=2\n\033[00m"));

        num_threads = DDEFAULT_NUM_THREADS;
    }
    if (argc>1)
        PresupostFitxatges = atoi(argv[1]);
...
void CalcularEquipOptim(long int PresupostFitxatges, TJugadorsEquip* MillorEquip)
{
    ...
    TEquip num_steps = end-first;
    TEquip step = end/num_steps;

    Intervals *intervals[num_threads];

    for (int h = 0; h < num_threads; ++h) {
        intervals[h] = (Intervals *) malloc(sizeof(struct Intervals));
        if(h>0){
            intervals[h]->begin = intervals[h-1]->end+step;
        } else{
            intervals[h]->begin = first;
        }
        TEquip block = (num_steps/(num_threads-h))*step;
        if (h==(num_threads-1)){
            intervals[h]->end=end;
        } else{
            intervals[h]->end = intervals[h]->begin+block;
        }
        intervals[h]->MillorEquip_parcial = Parcial[h];
        intervals[h]->PresupostFitxatges = 300;
        if(pthread_create(&Threads[h], NULL, (void *(*)(void *)) CalcularEquipOptim_thread, intervals[h])!=0){//pthread_create(&Threads[i], NULL, (void *(*)(void *)) funcion_barata, (void *)parameters)){
            //if 0 --> succes thread creation
            perror("ERROR THREAD");
            //TODO cancel threads
            exit(1);
        } else{
            //sprintf (cad,"CREATED THREAD --> %llXH to %llXH \n",intervals[h]->begin,intervals[h]->end);
            //write(1,cad,strlen(cad));
        }
        num_steps=num_steps-block;

    }

    // TODO FALTA CANCELAR ELS THREADS
    // TODO FALTA EL JOIN
    int max=0;
    for (int h = 0; h < num_threads; ++h) {
        if(pthread_join(Threads[h], NULL)){		//Espera a que termine la ejecucion del hilo j.
            printf("Error al hacer join\n");
            exit(1);
        }else{
...
void* CalcularEquipOptim_thread(Intervals *pIntervals) {
    TEquip first =pIntervals->begin;
    TEquip end = pIntervals->end;
    int PresupostFitxatges = pIntervals->PresupostFitxatges;
    TJugadorsEquip* MillorEquip = pIntervals->MillorEquip_parcial;
    //free(input);
    // -------------------------------------ZONA SEQUENCIAL------------------------------
    int MaxPuntuacio = -1;
    TJugadorsEquip MillorEquip_parcial;
