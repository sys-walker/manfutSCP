...
public class Manfut {
    ...
    public static final int DEFAULT_MANFUT_M=25000;

    public static Market.ManfutStats general_stats = new Market.ManfutStats();
    public static int num_threads;
    public static int remainingThreads = 0;
    public static int M;
    ...
    public static void main(String[] args)
    {
        ...
        // Procesar argumentos.
        if (args.length<2)
            throw new IllegalArgumentException("Error in arguments: ManFut <presupost> <fitxer_jugadors> <num_threads>");
        if (args.length>=3) {
            ...
            System.out.print(Error.color_red);
            System.out.print("No  <Num threads>, setting to defaults num_threads=2");
            System.out.println(Error.end_color);
            num_threads=DEFAULT_MANFUT_THREADS;
        }
        if (args.length==4) {
            if(Integer.parseInt(args[3])>0){
                ...
                System.out.print(Error.color_red);
                System.out.print("Negative numbers are not allowed, setting to defaults M=25000");
                System.out.println(Error.end_color);
                M=DEFAULT_MANFUT_M;
            }
        }else{
            System.out.print(Error.color_red);
            System.out.println("No  <M>, setting to defaults M=25000");
            System.out.println(Error.end_color);
            M=DEFAULT_MANFUT_M;
        }
